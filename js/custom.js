...
function stickyHeader() {
    if ($('.stricky').length) {
        ...
        if ($(window).scrollTop() > strickyScrollPos) {        	
            ...
            stricky[0].style.top = '0px';
            stricky.removeClass('stricky-fixed slideInDown animated');
            stricky.addClass('slideIn animated');
            $('.avoid-site-jump')[0].style.position = 'absolute'
...
// @TODO działa tylko lokalnie !!!!!!!!!!!!!!!!! 
function currentNav(){
    var path = window.location.pathname;
    // console.log(path)
    var links = $('nav.header-navigation .main-navigation > ul > li');
    for(var i=0; i < links.length; i++){
        var hrefs = links[i].getElementsByTagName('a');
        for(var j=0; j < hrefs.length; j++){
            if( hrefs[j].href.replace('http://localhost:8000', '') == path){
